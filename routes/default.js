...
// m_products
router.get('/products', block_access.isLoggedIn, block_access.moduleAccessMiddleware("products"), function(req, res) {
    var widgetPromises = [];
    // *** Widget module m_products | Do not remove ***
    Promise.all(widgetPromises).then(function(results) {
        var data = {};
        for (var i = 0; i < results.length; i++)
            for (var prop in results[i])
                data[prop] = results[i][prop];
        res.render('default/m_products', data);
    });
});
...
router.get('/home', block_access.isLoggedIn, block_access.moduleAccessMiddleware("home"), function(req, res) {
    var widgetPromises = [];
    // *** Widget call e_stock info start | Do not remove ***
	widgetPromises.push(new Promise(function(resolve, reject){
		models.E_stock.count().then(function(result){
			resolve({e_stock_info: result});
		});
	}));
	// *** Widget call e_stock info end | Do not remove ***

	// *** Widget call e_rail info start | Do not remove ***
	widgetPromises.push(new Promise(function(resolve, reject){
		models.E_rail.count().then(function(result){
			resolve({e_rail_info: result});
		});
	}));
	// *** Widget call e_rail info end | Do not remove ***

	// *** Widget call e_hanger info start | Do not remove ***
	widgetPromises.push(new Promise(function(resolve, reject){
		models.E_hanger.count().then(function(result){
			resolve({e_hanger_info: result});
		});
	}));
